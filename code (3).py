...
def picking():
    while game != 'over':
        num = 0
        removing = input(">  ")
        while len(removing) < 1 or len(removing) > 1:
            print(main)
            removing = input(">  ")
        if removing == "1":
            while main[0] not in deck[num]["name"]:
                num += 1
            if track.elixir < deck[num]["elixir"]:
                print("Not enough mana")
                return
            print(f"Sucessfully summoned {main[0]}")
            main.remove(main[0])
            track.elixir -= deck[num]["elixir"]
        elif removing == "2":
            while main[1] not in deck[num]["name"]:
                num += 1
            if track.elixir < deck[num]["elixir"]:
                print("Not enough mana")
                return
            print(f"Sucessfully summoned {main[1]}")
            main.remove(main[1])  
            track.elixir -= deck[num]["elixir"] 
        elif removing == "3":
            while main[2] not in deck[num]["name"]:
                num += 1
            if track.elixir < deck[num]["elixir"]:
                print("Not enough mana")
                return
            print(f"Sucessfully summoned {main[2]}")
            main.remove(main[2])
            track.elixir -= deck[num]["elixir"]
        elif removing == "4":
            while main[0] not in deck[num]["name"]:
            ...
            if track.elixir < deck[num]["elixir"]:
                print("Not enough mana")
                return
            print(f"Sucessfully summoned {main[3]}")
            main.remove(main[3])
            track.elixir -= deck[num]["elixir"]
...
thread_timestwo = threading.Thread(target=timesmana)
thread_elixir_count.start()
thread_timestwo.start()
while game != 'over':
    picking()